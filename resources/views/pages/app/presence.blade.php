...
@section('content')
  ...
  <section id="view-presensi" class="view">
    <div class="max-w-3xl mx-auto space-y-6">
      ...
      <!-- Header: Clock & Date -->
      <div class="bg-white dark:bg-gray-900 rounded-3xl border border-gray-200 dark:border-gray-800 p-6 sm:p-8 text-center shadow-sm">
        <h2 class="text-4xl sm:text-5xl font-black tracking-tight tabular-nums text-gray-900 dark:text-gray-100" id="bigClock">07:45:12</h2>
        <p class="text-sm text-gray-500 dark:text-gray-400 mt-2 font-medium" id="presensiDate">Rabu, 29 Juli 2026</p>
      </div>

      <!-- Location Verification Status -->
      <div class="bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-900/50 rounded-2xl p-4 sm:p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div class="flex items-center gap-4">
          <div class="w-12 h-12 rounded-full bg-emerald-100 dark:bg-emerald-900/40 text-emerald-600 flex items-center justify-center shrink-0">
            <svg class="w-6 h-6 shrink-0" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24">
        ...
        <div class="flex items-center gap-2 px-3 py-1.5 bg-emerald-100 dark:bg-emerald-900/40 rounded-full text-xs font-bold text-emerald-700 dark:text-emerald-400 shrink-0">
      ...
      <div class="bg-white dark:bg-gray-900 rounded-3xl border border-gray-200 dark:border-gray-800 p-6 sm:p-8">
        ...
        <div id="tabContent-face" class="space-y-6 block fade-in-up">
          ...
          <div class="relative w-full max-w-sm mx-auto aspect-[3/4] bg-gray-100 dark:bg-gray-950 rounded-[2rem] overflow-hidden flex items-center justify-center border-4 border-dashed border-gray-300 dark:border-gray-700">
            <!-- Simulated Camera Feed -->
            <img src="https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?q=80&w=600&auto=format&fit=crop" 
                 class="absolute inset-0 w-full h-full object-cover opacity-80 mix-blend-luminosity hover:mix-blend-normal transition duration-500" alt="Camera Feed">
                 
            <!-- UI Overlay for Face Guide -->
            <div class="absolute inset-0 pointer-events-none flex flex-col justify-between items-center py-6">
               <span class="px-3 py-1 bg-black/50 text-white text-[10px] font-bold rounded-full backdrop-blur-md">Mendeteksi wajah...</span>
               <!-- Scanning line -->
               <div class="w-3/4 h-1 bg-telkom-500 rounded-full shadow-[0_0_15px_rgba(242,59,59,0.8)] animate-pulse"></div>
            </div>
            
            <!-- Face Frame -->
            <div class="absolute inset-x-8 inset-y-16 border-2 border-white/60 rounded-[3rem] shadow-[0_0_0_9999px_rgba(0,0,0,0.6)] pointer-events-none"></div>
          </div>
        ...
        <div id="tabContent-manual" class="space-y-6 hidden fade-in-up">
          ...
          <form onsubmit="event.preventDefault(); showToast('Presensi Manual Berhasil Dikirim!', 'success');" class="max-w-md mx-auto space-y-5">
            <div>
              <label class="block text-xs font-bold text-gray-700 dark:text-gray-300 mb-1.5">Tipe Presensi</label>
              <select class="w-full px-4 py-2.5 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl outline-none focus:ring-2 focus:ring-telkom-500 text-sm">
            ...
            <div>
              <label class="block text-xs font-bold text-gray-700 dark:text-gray-300 mb-1.5">Catatan Pekerjaan (Opsional)</label>
            ...
            <div>
              ...
              <div class="w-full border-2 border-dashed border-gray-300 dark:border-gray-700 rounded-xl p-6 flex flex-col items-center justify-center text-center hover:bg-gray-50 dark:hover:bg-gray-800 transition cursor-pointer">
                <svg class="w-8 h-8 text-gray-400 mb-2" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24">
  ...
  <script>
    function switchTab(tabId) {
      ...
      // Reset classes
      const activeClass = ['bg-white', 'dark:bg-gray-800', 'text-telkom-600', 'dark:text-telkom-400', 'shadow-sm', 'font-bold'];
      const inactiveClass = ['text-gray-500', 'hover:text-gray-700', 'dark:hover:text-gray-300', 'font-semibold', 'bg-transparent'];
    ...
    function showToast(msg, type = 'success') {
      ...
      t.className =
        `toast ${colors[type]} text-white px-4 py-3 rounded-xl shadow-lg flex items-center gap-3 text-sm font-bold max-w-xs`;
      t.innerHTML = `<svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14" /><polyline points="22 4 12 14.01 9 11.01" /></svg><span>${msg}</span>`;
      c.appendChild(t);
      setTimeout(() => {
        t.remove();
      }, 3000);
